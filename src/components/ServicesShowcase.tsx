    ...
    <section className="relative bg-[#02040e] text-white">
      <div className="mx-auto max-w-[1680px] px-5 pb-4 pt-6 md:px-8 lg:px-10">
        ...
        <div className="relative mb-6 hidden lg:block">
          ...
          <div className="absolute left-[11%] top-0 h-[5px] w-[5px] -translate-y-1/2 rounded-full bg-white/80" />
          <div className="absolute left-[27%] top-0 h-[5px] w-[5px] -translate-y-1/2 rounded-full bg-white/80" />

          <div className="absolute right-0 top-0 flex -translate-y-1/2 items-center gap-2">
            <button
              onClick={() => scrollRow("left")}
              className="flex h-8 w-8 items-center justify-center rounded-full border border-white/10 bg-white/[0.04] text-white/75 transition hover:bg-white/[0.07]"
              aria-label="Назад"
            >
              ←
            </button>
        ...
        <div className="grid gap-4 xl:grid-cols-[230px_minmax(0,1fr)]">
          ...
          >
            {services.map((item) => (
              <article
                key={item.title}
                className="min-h-[150px] min-w-[190px] rounded-[22px] border border-white/8 bg-[linear-gradient(180deg,rgba(255,255,255,0.045)_0%,rgba(255,255,255,0.018)_100%)] p-5 shadow-[0_14px_35px_rgba(0,0,0,0.16)] backdrop-blur-[18px]"
              >
                <div className="mb-5 flex h-10 w-10 items-center justify-center rounded-2xl border border-[#4b8dff]/20 bg-[#4b8dff]/10 text-[#6ea3ff]">
                  {item.icon}
                </div>
                ...
                <div className="mt-5 flex justify-end">
                  <span className="inline-flex h-6 w-6 items-center justify-center rounded-full border border-white/10 bg-white/[0.04] text-[12px] text-white/70">
                    →
                  </span>
                </div>
              </article>
